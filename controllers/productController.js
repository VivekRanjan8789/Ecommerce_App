  ...
  try {
    ...
    switch (true) {
      ...
      case !photo || photo.size > 100000:
        return res
          .status(200)
          .send({ error: "photo0 is required less than 1 MB" });
    }
...
export const filterProductController = async (req, res) => {
  try {
    ...
    if (radio.length)
      arg.price = { $gte: Number(radio[0]), $lte: Number(radio[1]) };
...
export const braintreeTokenController = async (req, res) => {
  try {
    ...
    return res.status(500).send({
      success: false,
      message: "error while generating braintoken",
      error,
    });
  }
};
...
export const braintreePaymentController = async (req, res) => {
  try {
    ...
    console.log("total price in controller is: ", total);
    ...
    return res.status(500).send({
      success: false,
      message: "error while taking orders",
      error,
    });
  }
};
